...
import ku.piii2017.businesslogic5.*;
...
//        label.setText("Hello World!");
    ...
    @FXML
    private void removeDuplicatesT1(ActionEvent event){
        removeDuplicates(tableView1); 
    }
     @FXML
    private void removeDuplicatesT2(ActionEvent event){
        removeDuplicates(tableView2); 
    }
    
    private void removeDuplicates(TableView<MediaItem> refToTableView){
        
        DuplicateFinder dF = new DuplicateFindFromID3();
        Set<MediaItem> tableCollection = new HashSet(refToTableView.getItems());
        if (tableCollection.size()> 0){
        // get unique items
        Set<MediaItem> result = dF.getAllButNoDuplicates(tableCollection);
        
         ObservableList<MediaItem> dataForTableViewAndModel
                = FXCollections.observableArrayList(result);
        refToTableView.getItems().clear();
        refToTableView.setItems(dataForTableViewAndModel);
        }
      ...
      private void alertUser(){
        ...
        alert.setHeaderText("There are no duplicates to remove");
